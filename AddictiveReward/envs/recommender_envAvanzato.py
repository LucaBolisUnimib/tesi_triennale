...
class Recommender_envAvanzato:
    ...
    def set_probabilities(self, TRANSITION_PROBABILITIES):
        ...
        if(self.current_arm == self.ARM_A):
            # P(s=8|s=7,a=aD) = 1.0
            TRANSITION_PROBABILITIES[END_NEUTRAL_STATE][aD][DRUG_STATE] = [1.0, reward]
            TRANSITION_PROBABILITIES[END_NEUTRAL_STATE][aD][S0] = [0.0, 0]
        else: #elif(self.current_arm == self.ARM_B):
            # P(s=4|s=7,a=aD) = 1.0
            TRANSITION_PROBABILITIES[END_NEUTRAL_STATE][aD][S0] = [1.0, reward]
            TRANSITION_PROBABILITIES[END_NEUTRAL_STATE][aD][DRUG_STATE] = [0.0, 0]
